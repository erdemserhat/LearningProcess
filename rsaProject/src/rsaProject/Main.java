...
public class Main {
	...
	/*
	 * Generating Keys... select two prime numbers (P,Q) calculate product of P andQ
	 * caculate totient=(P-1)*(Q-1) then select a public key (E) -Must be prime
	 * -Must be less than totient -Must not be factor of the totient select a
	 * private key (D) Product of D and E, dividen by T must result in a remainder
	 * of 1 so (D*E)%t==1
	 * 
		...
		for (BigInteger i = new BigInteger("1"); totient.compareTo(i) == 1; i = i.add(BigInteger.ONE)) {
			...
			for (BigInteger k = new BigInteger("2"); k.compareTo(i.sqrt()) == -1; k = k.add(BigInteger.ONE)) {
				if (i.mod(k).compareTo(BigInteger.ZERO) == 0)
					control = false;
				if (i.mod(totient).compareTo(BigInteger.ZERO) == 0)
					control = false;
	...
	private static void PrivateKeyGenerator() {
		...
		for (i = new BigInteger("1"); i.multiply(publicKey).mod(totient).compareTo(BigInteger.ONE) != 0; i = i
				.add(BigInteger.ONE)) {
	...
	public static void FirstMenu() {
		...
		System.out
				.println("Firstly, you need to define tokens and keys let's get started by definening p and q values");
	...
	public static void EncryptionMenu() { // Encryption Menu
		...
		// System.out.printf("to decryption use = private key: %d , publickey: %d , p
		// value is %d , q value is %d"
	...
	public static void DecryptionMenu() { // DECRYPTİON MENU
		...
		int[] chipperInt = new int[arrayLength];
		//BigInteger[] messageInt = new BigInteger[arrayLength];
		String text = "";

		System.out.println("Enter the your chipper text");
		text = input.nextLine();
		int k = 0;
		char chracters = text.charAt(0);
		for (int i = 0; i <arrayLength; i++) {

			while (chracters != ',') {
				
				chipperInt[i]=chipperInt[i]+text.charAt(k);
				k++;
				chracters = text.charAt(k);

			}

		}
		
		for(int i =0; i<arrayLength; i++) System.out.println(chipperInt[i]);
	 ...
	 * messageInt[i]=Decryption(BigInteger.valueOf(chipperInt[i])); } for(int i =0;
	 * i<= arrayLength-1; i++) { char ch = (char) messageInt[i].intValue();
	 * //converting ascii code to chracter. text =text+ch; // ordering every
	 * chracter. }
	 * 
	 * System.out.println("your message is: " +text); System.out.println("");
	 * MainMenu();
	 * 
	 * }
	 */
